...
func TestTrim(t *testing.T) {
	...
	// Test 2: Trim first entry (key "1737238769766", seqNum 0)
	tree.Trim(Key("1737238769766"), 0)
	if tree.Length() != 7 {
	...
	// Test 3: Trim one of the duplicate entries (key "1737238769806", seqNum 0)
	// This should remove entry 3 but keep entry 4
	tree.Trim(Key("1737238769806"), 0)
	if tree.Length() != 5 {
...
func TestTrimWithMultipleValuesPerKey(t *testing.T) {
	...
	// Trim first two values from first key
	tree.Trim(Key("1000000000000"), 1)
	if tree.Length() != 4 {
	...
	// Trim all of first key and part of second
	tree.Trim(Key("2000000000000"), 0)
	if tree.Length() != 2 {
